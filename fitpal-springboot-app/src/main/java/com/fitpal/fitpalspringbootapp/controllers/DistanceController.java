...
public class DistanceController {
    ...
    @GetMapping("/daily")
    public ResponseEntity<DistanceResponse> getDailyDistance(@RequestParam String userId, @RequestParam String date) {
        double distance = distanceService.getDailyDistance(userId, date);
        return ResponseEntity.ok(new DistanceResponse(distance));
    }

    @GetMapping("/weekly")
    public ResponseEntity<DistanceResponse> getWeeklyDistance(@RequestParam String userId, @RequestParam String date) {
        double distance = distanceService.getWeeklyDistance(userId, date);
        return ResponseEntity.ok(new DistanceResponse(distance));
    }

    @GetMapping("/monthly")
    public ResponseEntity<DistanceResponse> getMonthlyDistance(@RequestParam String userId, @RequestParam String month) {
